...
export class SerpApiLinkedInScraper {
  ...
  constructor(apiKey?: string) {
    ...
    if (!this.apiKey) {
      console.warn('SERPAPI_KEY not found. SerpAPI LinkedIn scraper will not work.');
    }
  }

  getSourceName(): JobSource {
    return 'linkedin' as JobSource;
  }
    ...
    try {
      ...
      if (response.error) {
        throw new ScraperError(
          this.getSourceName(),
          `SerpAPI LinkedIn error: ${response.error}`,
      ...
      if (response.jobs && Array.isArray(response.jobs)) {
        jobs = response.jobs
          .slice(0, maxResults)
          .map((job: SerpApiLinkedInJob) => this.transformJob(job))
      ...
      console.error('SerpAPI LinkedIn scraper error:', error);
  ...
  private transformJob(serpJob: SerpApiLinkedInJob): RawJob | null {
    ...
    return {
      ...
      url: jobUrl,
      source: this.getSourceName(),
      jobType: serpJob.job_type?.toLowerCase(),
      postedDate,
      scrapedAt: new Date(),
      metadata: {
